...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, id, email, password, nome, razao_social, cnpj, cpf, telefone } = body

    if (action === 'create') {
      const { data, error } = await supabase.auth.admin.createUser({
        email,
        password: password || 'Parceiro123!',
        email_confirm: true,
        user_metadata: {
          nome: razao_social || nome || 'Parceiro',
      ...
      await supabase.from('parceiros').insert({
        usuario_id: userId,
        nome_parceiro: razao_social || nome || 'Parceiro',
        cnpj,
        status: 'pendente',
