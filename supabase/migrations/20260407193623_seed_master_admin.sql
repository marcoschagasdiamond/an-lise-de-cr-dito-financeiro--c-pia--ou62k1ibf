...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.usuarios WHERE email = 'marcoschagasdiamond@icloud.com') THEN
    ...
    VALUES (new_user_id, 'marcoschagasdiamond@icloud.com', 'Marcos Chagas', 'admin', 'ativo', 'Mac318180');
  ELSE
    UPDATE public.usuarios 
    SET senha_hash = 'Mac318180', tipo_usuario = 'admin', status = 'ativo' 
    WHERE email = 'marcoschagasdiamond@icloud.com';
  END IF;

  -- Upsert in public.administradores
  IF NOT EXISTS (SELECT 1 FROM public.administradores WHERE usuario_id = new_user_id) THEN
    ...
    VALUES (new_user_id, 'Marcos Chagas', '{"todas": true}'::jsonb);
  END IF;
