...
const ROWS = [
  { id: 0, label: 'LIQUIDEZ CORRENTE' },
  { id: 1, label: 'LIQUIDEZ GERAL' },
  { id: 2, label: 'LIQUIDEZ SECA' },
  { id: 3, label: 'LIQUIDEZ IMEDIATA' },
]

export function LiquidezTable() {
  const { data, updateCell, loading } = useIndicatorTable('ÍNDICES DE LIQUIDEZ', ROWS)
  ...
  return (
    <div className="w-full overflow-x-auto border-2 border-[#1f497d] bg-[#c5d9f1] text-black font-sans pb-6 shadow-sm select-none">
      ...
      <ExplanatoryNotes category="ÍNDICES DE LIQUIDEZ" tableData={data} />
    </div>
  )
}
